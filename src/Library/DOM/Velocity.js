...
(function(window,KUBE){
    ...
    function VelocityKUBE(_DomJack){
        ...
        $API = {
            ...
            "Delay":Delay,
            "Display":Display,
            "MobileHardwareAcceleration":MobileHardwareAcceleration,
            "Stop":Stop,
            "Reverse":Reverse,
            "Animate":Animate,
            "SlideUp": SlideUp,
        ...
        function initVelocity(){
            ...
            if(KUBE.Is(_DomJack,true) === 'DomJack'){
                //debugger;
                V = _DomJack;
                //window.velocity.Utilities(_DomJack.GetNode());
                //We need to destroy this when our _DomJack is destroyed, and likewise Emit an event.
            }
            else{
                throw new Error('Valid DomJack must be passed into Velocity');
            }
        }

        function Duration(_ms){
            if(KUBE.Is(_ms) === 'number'){
                OptionStore.duration = _ms;
            }
            return $API;
        }

        function Easing(_ease){
            OptionStore.easing = _ease;
            return $API;
        }
        ...
        function Begin(_callback){
            if(KUBE.Is(_callback) === 'function'){
                OptionStore.begin = _callback;
            }
            return $API;
        }
        ...
        function Complete(_callback){
            if(KUBE.Is(_callback) === 'function'){
                OptionStore.complete = _callback;
            }
            return $API;
        }

        function Loop(_loops){
            OptionStore.loop = false;
            if(KUBE.Is(_loops) === 'number'){
                OptionStore.loop = _loops;
            }
            return $API;
        }

        function Delay(_ms){
            OptionStore.delay = false;
            if(KUBE.Is(_ms) === 'number'){
                OptionStore.delay = _ms;
            }
            return $API;
        }

        function Display(_cssDisplay){
            OptionStore.display = false;
            if(KUBE.Is(_cssDisplay) === 'string'){
                OptionStore.display = _cssDisplay;
            }
            return $API;
        }

        function MobileHardwareAcceleration(_bool){
            OptionStore.mobileHA = (_bool ? true : false);
            return $API;
        }

        function Stop(){
            Velocity.animate(_DomJack.GetNode(),"stop");
        }

        function Reverse(_newDuration){
            if(_newDuration){
                Velocity.animate(_DomJack.GetNode(),"reverse",{"duration":_newDuration});
            }
            else{
                Velocity.animate(_DomJack.GetNode(),"reverse");
            }
        }
        ...
        function FadeIn(){
            Velocity.animate([_DomJack.GetNode()],"fadeIn",OptionStore);
        }

        function FadeOut(){
            Velocity.animate([_DomJack.GetNode()],"fadeOut",OptionStore);
        }

        //CSS supported properties
        function Top(_top){
            PropertyStore.top = _top;
            return $API;
        }

        function Right(_right){
            PropertyStore.right = _right;
            return $API;
        }

        function Bottom(_bottom){
            PropertyStore.bottom = _bottom;
            return $API;
        }

        function Left(_left){
            PropertyStore.left = _left;
            return $API;
        }

        function Display(_display){
            OptionStore.display = _display;
            return $API;
        }

        function Opacity(_opacity){
            PropertyStore.opacity = _opacity;
            return $API;
        }
