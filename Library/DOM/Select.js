/* 
 * Name: Select
 * Type: KUBEFactoryClass
 */
(function(KUBE){
	"use strict";
	
	var Sizzle = (function(){
		/* Sizzle cause it's awesome */
		var i,
			cachedruns,
			Expr,
			getText,
		...
		function tokenize( selector, parseOnly ) {
			...
			while ( soFar ) {
				...
				for ( type in Expr.filter ) {
                    if(!Expr.hasOwnProperty(type)){continue;}
	...
	KUBE.LoadFactory('/Library/DOM/Select', Select, ['/Library/DOM/DomJack','/Library/Extend/Object']);
		...
		function DJSizzle(_array){
			...
			if(KUBE.Is(_array) === 'array'){
				for(i=0;i<_array.length;i++){
					$return.push(KUBE.Class('/Library/DOM/DomJack')(_array[i]));
				}
			}
			return $return;
		}
